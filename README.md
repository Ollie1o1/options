...
**Probability Lab — the market's implied distribution vs *your* view (descriptive, not gated).**
`python -m src.probability_lab AAPL --drift +3% --vol-mult 0.9` (or menu `[12] PROB LAB`) reads the **risk-neutral density** the option chain implies — the market's full probability distribution for where the stock lands at expiry — by taking the Breeden-Litzenberger second derivative (`q(K) = e^{rT} ∂²C/∂K²`) of the call curve reconstructed from the fitted **SVI smile**. You then tilt it into your own view with a directional **drift** and a **vol multiplier**; the tilt re-centers and re-scales the market's *own* density, preserving its skew rather than replacing it with a Gaussian. It prints: the market vs your `P(S_T > level)` at each key level (the **edge** column is where you disagree with what's priced), and the listed **long call / long put / call- and put-debit verticals ranked by EV under your view**, each with an `EV(mkt)` contrast column. This is arithmetic on the live chain plus a stated view — it sits in the descriptive layer alongside IV rank and Greeks, **not** the predictive gate. `EV(mkt) ≈ 0` on a structure means you're buying consensus; a positive `EV(view)` with a negative or near-zero `EV(mkt)` is where your view is actually differentiated. Model caveats are printed with every run: the RND is smoothed off delayed data, and the view EV is only as good as your view.
