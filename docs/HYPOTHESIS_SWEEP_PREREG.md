...
Locked 2026-09-19. Reflects the design spec's corrected, internally-consistent
table in `docs/superpowers/specs/2026-09-19-hypothesis-sweep-design.md` — that
spec is the source of truth; this file is the pointer `src/hypothesis_sweep.py`
...
is a new preregistration, not an edit to this one. (Not a byte-identical
copy of the spec's table — this file's H3 row wording differs slightly from
the spec's; both state the same locked hypothesis.)
...
**Known limitation, recorded 2026-09-21 (final-review fix wave): H3 is
currently unanswerable under this engine.** `backtest_ticker_vertical`'s
per-leg friction lookup (`src/backtest_spreads.py`) is keyed on the
function's own fixed `target_delta`/`wing_delta`/`entry_dte` parameters —
constant across every trade in a run — not each trade's realized delta/DTE.
`spread_score` (`comp[SPREAD_IDX]`) is therefore the same number for every
trade, its variance is zero, and `rank_ic`/`cluster_bootstrap_ci` both
return `None` (undefined, not zero). `run_h3_spread_weight` detects this and
refuses with `reason="ic_undefined_constant_feature"` rather than reporting
a false "null" — this is knowable without running anything, and fixing the
underlying per-trade friction lookup is a design change belonging to a new
preregistration batch, not an edit to H1-H6's already-locked simulation
logic.
