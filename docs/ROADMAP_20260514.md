<!-- Historical document. Filed 2026-08-18; written 2026-05-14. -->

> # SUPERSEDED — read this header before acting on anything below
>
> This roadmap was written on **2026-05-14** and is preserved as a record of
> what was planned then, not as a plan for now. It originated on the branch
> `claude/debug-code-issues-WXFjr` (commit `201cdeb`), which was never merged;
> the content is filed here so the branch can be pruned without losing it.
>
> **It predates the measurement work, and its central premise did not survive.**
>
> Part 3 proposes twelve new entry-feature signals. `docs/ATTRIBUTION_20260808.md`,
> written three months later, reports: *"Mostly a negative result... No entry
> feature"* predicts outcome — and the strongest correlate in that table is
> `friction_pct_credit` at **-0.3088 (clustered t -4.39)**. In this book, cost
> predicts returns and entry features do not. Several Part 3 items are also
> individually disproven since: single-name equity VRP found no edge (3.1),
> `skew_25d` was retired by the residualised IC screen (3.6), and the overlays
> including PCR measured IC ~ 0 (3.12). Adding twelve unregistered signal
> searches is also the multiple-comparisons trap that `PREREG_OPTIONSDX.md` and
> `PREREG_SECTOR_CONDITIONING_20260813.md` were adopted to prevent.
>
> **1.1 (Kelly sizing) is the item to be most careful with.** Kelly sizes UP on
> a believed edge. The Long Call gate reads STOP at 4% posterior and the
> post-restart cohort is -$8,480 over 19 closed trades; applying Kelly to a
> negative-edge system scales the losses. The book's earlier loss was already
> measured to be a sizing artifact.
>
> **What still stands:** 1.2 (portfolio Greeks limits at entry — risk control,
> not alpha), 1.6 (Greeks P&L attribution at close — measurement, not
> prediction), 1.3 and 1.4 (backtest/live mismatches, the same label-vs-reality
> class of defect repaired across 2026-08-17/18), and most of Part 4's hygiene
> items. Note 4.1 is largely served already by the WORTH breakeven margin.
>
> **The "profit impact" column in the execution table is an unvalidated
> assertion.** Nothing in it was measured. Treat the ordering as an opinion
> from May 2026.
