...
class TestRealizedPnl(unittest.TestCase):
    def test_long_debit_with_quantity(self):
        r = realized_pnl(entry=2561.03, exit=3115.05, qty=0.39, side="long", structure="debit")
    ...
    def test_short_premium_expired_worthless(self):
        r = realized_pnl(entry=240.0, exit=0.0, qty=1.0, side="short", structure="debit")
    ...
    def test_credit_spread_close(self):
        r = realized_pnl(entry=250.0, exit=90.0, qty=2.0, side="short", structure="credit")
    ...
    def test_qty_defaults_to_one(self):
        r = realized_pnl(entry=100.0, exit=130.0, qty=None, side="long", structure="debit")
